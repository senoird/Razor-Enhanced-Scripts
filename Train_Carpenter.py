...
#
# What it does:
# 1. Restocks boards from a specified crate when you run low using a new, safer method.
...
# 4. After crafting an item, it uses an axe to destroy it to make room.
...
# 3. Have a crate with boards, a pouch with saws, and an axe in your backpack.
...
def restock_boards(crate_serial):
    """Restocks boards from the supply crate using precise weight calculation."""
    ...
    # Calculate how many boards to move based on 90% of available weight.
    # Assumes 1 board = 1 stone.
    available_weight = Player.MaxWeight - Player.Weight
    boards_to_move = int(available_weight * 0.90)
    
    if boards_to_move <= 0:
        Misc.SendMessage(">> You are too heavy to restock more boards.", 138)
        return True # Not an error, just can't carry more

    # Move the calculated number of boards
...
def destroy_item(axe_serial, item_graphic, pouch_serial):
    """Finds and destroys the specified item, looking in the tool pouch."""
    # Search for the item inside the specified pouch.
    # The 'True' flag searches recursively, in case of nested bags.
    item_to_destroy = Items.FindByID(item_graphic, -1, pouch_serial, True)
    
    if item_to_destroy is None:
        # As a fallback, check the main backpack as well.
        item_to_destroy = Items.FindByID(item_graphic, -1, Player.Backpack.Serial, True)

    if item_to_destroy:
        Misc.SendMessage(">> Destroying last crafted item...", 78)
        Items.UseItem(axe_serial)
        if Target.WaitForTarget(2000):
            Target.TargetExecute(item_to_destroy.Serial)
            Misc.Pause(1000)
    else:
        Misc.SendMessage(">> Could not find crafted item to destroy.", 138)
...
while True:
    ...
    # Destroy the crafted item if successful
    if not Journal.Search("You fail to create"):
        destroy_item(axe_serial, craft_details["graphic"], saw_pouch_serial)
    else:
